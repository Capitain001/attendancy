#!/usr/bin/env tsx
// scripts/generate/service/create-service.ts
//
...
//
// Usage :
//   npx tsx scripts/generate/service/create-service.ts <name> --model=<Model>
//   npx tsx scripts/generate/service/create-service.ts --name=<name> --model=<Model> [options]
//
// Options :
...
//
// Exemples :
//   npx tsx scripts/generate/service/create-service.ts course-teacher --model=CourseTeacher
//   npx tsx scripts/generate/service/create-service.ts --name=course-teacher --model=CourseTeacher --prefix=ue --soft-delete --minimal
//
...
// (parseArgs : voir ./cli-utils.ts, partagé avec service.ts — ne pas
// redupliquer ici.)
...
function printUsageAndExit(message?: string): never {
  ...
  console.error(
    [
      ...
      "",
      "Exemple:",
      "  npx tsx scripts/generate/service/create-service.ts course-teacher --model=CourseTeacher --prefix=ue --soft-delete --minimal",
    ].join("\n"),
  );
  process.exit(1);
}
...
// ─── Helpers de casse ──────────────────────────────────────────────────────────
// (voir ./cli-utils.ts, importé ci-dessus — partagé avec service.ts)
...
// (importé ci-dessus, partagé avec service.ts).
...
// importé ci-dessus — partagé avec service.ts, ne pas redupliquer)
