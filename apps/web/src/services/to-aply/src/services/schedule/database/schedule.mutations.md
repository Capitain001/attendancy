...
/**
 * Supprime l'ensemble des schedules associés à une règle de récurrence.
 * Invalide le cache pour chaque classe impactée (une règle peut couvrir
 * plusieurs classes/groupes selon le modèle de récurrence).
 */
export async function deleteSchedulesByRule(ruleId: string, orgId: string) {
  ...
  await invalidateEvent('SCHEDULE_REMOVED', orgId)
  await Promise.all(
...
/**
 * Supprime uniquement les séances futures et non confirmées d'une règle
 * de récurrence — typiquement pour modifier "les prochaines séances"
 * sans toucher à l'historique déjà passé/confirmé.
 */
