...
/**
 * Annuaire étudiants pour la Direction (P-14/P-15).
 * Liste administrative scopée org. Inclut les étudiants désactivés.
 * Le statut vient de UserOrganization.status (scope org).
 */
export async function getDirectionStudents(orgId: string, classId?: string) {
  'use cache'
  cacheTag(CACHE.STUDENT(orgId))
  if (classId) {
    cacheTag(CACHE.STUDENT(orgId, classId))
  ...
  const rows = await prisma.student.findMany({
    ...
    select: {
      ...
      _count: { select: { childrenRelations: true } },
      childrenRelations: {
        select: {
          relation: true,
          parent: {
