...
export function CollapseSection({ label, children, defaultOpen = true, count }: {
  label: string; children: React.ReactNode; defaultOpen?: boolean; count?: number;
}) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div className="mt-6">
      <button onClick={() => setOpen(v => !v)} className="flex w-full items-center gap-3 text-left mb-3">
        ...
        <svg className={`size-3 text-muted-foreground/50 transition-transform duration-200 flex-shrink-0 ${open ? 'rotate-180' : ''}`}
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
            ...
            className="overflow-hidden">{children}</motion.div>
        )}
      </AnimatePresence>
    </div>
  );
