...
function useCarousel() {
  ...
  if (!context) throw new Error('useCarousel must be used within an CarouselProvider');
  return context;
}
        ...
        {Array.from({ length: itemsCount }, (_, i) => (
          <button
            ...
            className={cn(
              ...
              index === i ? 'w-4 bg-zinc-950 dark:bg-zinc-50' : 'w-2 bg-zinc-900/30 dark:bg-zinc-100/30',
              classNameButton
            )}
          />
        ))}
...
function CarouselItem({ children, className }: CarouselItemProps) {
  return (
    <motion.div className={cn('w-full min-w-0 shrink-0 grow-0 overflow-hidden h-full', className)}>
      {children}
    </motion.div>
  );
}
