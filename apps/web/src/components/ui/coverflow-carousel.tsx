      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {active.meta && active.meta.length > 0 && (
            <div className="mt-8 flex w-full max-w-[240px] justify-between text-[13px]">
              {active.meta.map((row, idx) => (
                <div key={row.label} className={cn("flex flex-col gap-1", idx === 1 ? "text-right" : "text-left")}>
                  <span className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">{row.label}</span>
