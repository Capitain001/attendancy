...
export function Gauge({
  ...
  const getGaugeConfig = () => {
    switch (gaugeType) {
      case "half":
        return { startAngle: -90, endAngle: 90, circumferenceFactor: 0.5, viewBox: `0 25 ${circleSize} 50` }
      case "quarter":
        return { startAngle: 0, endAngle: 90, circumferenceFactor: 0.25, viewBox: `25 25 50 50` }
      default:
        return { startAngle: -90, endAngle: 270, circumferenceFactor: 1, viewBox: `0 0 ${circleSize} ${circleSize}` }
    }
  }
  ...
  const getColor = (colorProp: typeof primary, isSecondary = false) => {
    ...
    if (!colorProp) {
      ...
      return strokePercent <= 25
        ...
        : strokePercent <= 50 ? defaultColors.warning
        : strokePercent <= 75 ? defaultColors.info
  ...
  const primaryOpacity = () => {
    if (offsetFactor > 0 && strokePercent < gapPercent * 2 * offsetFactor && strokePercent < gapPercent * 2 * offsetFactorSecondary) return 0
    return 1
  }
  ...
  const secondaryOpacity = () => {
    if (
      ...
      (offsetFactor > 0 && strokePercent > 100 - gapPercent * 2 * offsetFactor && strokePercent > 100 - gapPercent * 2 * offsetFactorSecondary)
    ) return 0
    return 1
  }
  ...
  const glowStyles = glowEffect
    ? { filter: `drop-shadow(0 0 2px ${primaryStroke}80) drop-shadow(0 0 6px ${primaryStroke}60) drop-shadow(0 0 12px ${primaryStroke}40) drop-shadow(0 0 20px ${primaryStroke}20)` }
    : {}
  ...
  return (
    <div className="relative inline-block">
      ...
      >
          ...
          {segments?.map((seg, i) =>
            seg.pattern ? (
              <pattern
                ...
                width={seg.pattern === "dots" ? "4" : seg.pattern === "dashed" ? "6" : "5"}
                height={seg.pattern === "dots" ? "4" : seg.pattern === "dashed" ? "6" : "5"}
                patternTransform={seg.pattern === "skew" ? "rotate(-45)" : undefined}
              >
                <rect width="100%" height="100%" fill={seg.color ?? primaryStroke} />
                {seg.pattern === "skew" && <line x1="0" y1="0" x2="0" y2="5" stroke="black" strokeWidth="1" strokeOpacity="0.2" />}
                {seg.pattern === "dashed" && <line x1="0" y1="3" x2="4" y2="3" stroke="white" strokeWidth="1.5" strokeOpacity="0.25" />}
                {seg.pattern === "dots" && <circle cx="2" cy="2" r="0.8" fill="white" fillOpacity="0.3" />}
              </pattern>
            ) : null
          )}
        </defs>
        ...
        {multiRing?.enabled && multiRing.rings?.map((ring, index) => (
          <circle
            key={`ring-${index}`}
            cx={circleSize / 2}
            cy={circleSize / 2}
        ...
        {!segments || segments.length === 0 ? (
          ...
          (() => {
            const capExtensionPx = strokeWidth / 2
            const internalGapPx = (gapPercent * adjustedPercentToPx) * 0.4
            const totalValueInSegments = segments.reduce((s, seg) => s + seg.value, 0)
            const clampedTotal = strokePercent
            let cursorPx = 0

            return segments.map((seg, i) => {
              const segRatio = seg.value / totalValueInSegments
              const currentSegFullPx = (segRatio * clampedTotal) * adjustedPercentToPx
              const drawPx = Math.max(currentSegFullPx - (capExtensionPx * 2) - internalGapPx, 0)
              const baseAngle = -90 + (gapPercent * offsetFactor) * percentToDegree
              const startOffsetPx = cursorPx + capExtensionPx + (internalGapPx / 2)
              const startOffsetAngle = (startOffsetPx / adjustedPercentToPx) * percentToDegree
              const transform = `rotate(${baseAngle + startOffsetAngle}deg)`
              const stroke = seg.pattern ? `url(#seg-pat-${i})` : (seg.color ?? primaryStroke)
              const result = (
                <circle
                  key={i}
                  cx={circleSize / 2}
                  cy={circleSize / 2}
                  r={radius}
                  style={{ ...circleStyles, strokeDasharray: `${drawPx} ${adjustedCircumference}`, transform, stroke, opacity: primaryOpacity(), transformOrigin: "50% 50%" }}
                />
              )
              cursorPx += currentSegFullPx
              return result
            })
          })()
        )}
        ...
        {thresholds?.map((threshold, index) => {
          ...
          const angle = (thresholdPercent / 100) * (gaugeConfig.endAngle - gaugeConfig.startAngle) + gaugeConfig.startAngle
