...
export async function getAuthorization(
  ...
  if (requiredRole) {
    ...
    const isRoleAllowed = requiredRoles.some((role) =>
  ...
  // ===== FONCTIONS =====
  // ADMIN bypasse les fonctions
  if (
    requiredFunction &&
    userRole !== "ADMIN" &&
    userFunction !== requiredFunction
  ) {
    return {success: false, error: `Fonction ${userFunction} insuffisante (requise: ${requiredFunction})`,
    };
  }
